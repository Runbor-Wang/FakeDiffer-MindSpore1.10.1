...
MODELS_PATH = {"ReconstructionXception": "model/network/ReconstructionXception.py", "FakeDiffer": "model/network/FakeDiffer.py"}


def exp_recons_loss(recons_real, recons_fake, total, lambda_1, lambda_2):
    x, y = total
    loss_real = ms.tensor(0., device=y.device)
    loss_fake = ms.tensor(0., device=y.device)
    real_index = torch.where(1 - y)[0]
    fake_index = torch.where(y)[0]
    # print("25 x.size() :", x.size())
    if recons_real is not None:
        # print("26 recons_real.size() :", recons_real.size())
        for real_rec in recons_real:
            # print("28 real_rec.size() :", real_rec.size())
            if real_index.numel() > 0:
                real_x = torch.index_select(x, dim=0, index=real_index)
                # print("31 real_x.size() :", real_x.size())
                ...
                # print("34 real_rec.size() :", real_rec.size())
                loss_real += torch.mean(torch.abs(real_rec - real_x))
                # print("36 loss_real :", loss_real)
    else:
        loss_real = 0.

    if recons_fake is not None:
        for fake_rec in recons_fake:
            # print("39 fake_rec.size() :", fake_rec.size())
            if fake_index.numel() > 0:
                fake_x = torch.index_select(x, dim=0, index=fake_index)
                ...
                loss_fake += torch.mean(torch.abs(fake_rec - fake_x))
    else:
        loss_fake = 0.

    return lambda_1 * loss_real + lambda_2 * loss_fake
