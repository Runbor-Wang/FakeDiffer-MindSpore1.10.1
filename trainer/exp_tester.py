...
TestModeList = ["reconstruction", "differ", "reconstruction_differ"]
...
class ExpTester(AbstractTrainer):
    ...
    def _test_settings(self, model_cfg=None, data_cfg=None, config_cfg=None):
        # load test dataset
        self.test_mode = config_cfg["test_mode"]
        if self.test_mode not in TestModeList:
            raise ValueError(f"Test mode must be in {TestModeList}, but found {self.test_mode}.")
        ...
        self.test_loader = data.DataLoader(self.test_set, shuffle=False, batch_size=self.test_batch_size)

        self.run_id = config_cfg["id"]
        self.pretrain_reconstruction_dir = config_cfg.get("pretrain_reconstruction_dir", None)
        self.pretrain_reconstruction_pth = config_cfg.get("pretrain_reconstruction_pth", None)
        self.pretrain_differ_pth = config_cfg.get("pretrain_differ_pth", None)

        self.ckpt_fold = config_cfg.get("ckpt_fold", "runs_2024-07-20-23:57:38")  # #################################
        self.dir = os.path.join(self.ckpt_fold, self.model_differ_name, self.run_id)
        # "/root/autodl-tmp/FakeDiffer/runs_2024-07-20-23:57:38/FakeDiffer/Celeb-DF"
        # self.model_reconstruction_name = model_cfg.pop("recons_name")
        # self.model_differ_name = model_cfg.pop("differ_name")

        # load model
        # self.num_classes = model_cfg["num_classes"]
        self.model_reconstruction = load_model(self.model_reconstruction_name)()
        self.model_differ = load_model(self.model_differ_name)(self.num_classes, self.model_reconstruction.encoder)
        # self.model = load_model(self.model_name)(**model_cfg)

        # load loss
        self.loss_criterion = get_loss(config_cfg.get("loss", None))
        self.recons_loss_criterion = get_loss("Reconstruction Stage", config_cfg.get("recons_loss", None), device=self.device)
        self.differ_loss_criterion = get_loss("Differ Stage", config_cfg.get("differ_loss", None), device=self.device)
