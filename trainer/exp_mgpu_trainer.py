...
from mindspore.nn.utils import data
from mindspore.cuda.amp import autocast, GradScaler
...
class ModelTrainer(AbstractTrainer):
    ...
    def _train_settings(self, model_cfg, data_cfg, config_cfg):
        ...
        # self.train_loader = data.DataLoader(self.train_set, shuffle=False, sampler=self.train_sampler,  # ############
        ...
        #     # wrapped with data loader
        #     self.val_loader = data.DataLoader(self.val_set, shuffle=True, num_workers=data_cfg.get("num_workers", 4),
        ...
        if not self.debug:
            ...
            self.dir = os.path.join("runs_shishi", self.model_name, self.run_id)
            # print("87 self.dir", self.dir)
        ...
        mindspore.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:23456', world_size=1, rank=0)
    ...
    def _save_ckpt(self, step, best=False):
        ...
        mindspore.save({"step": step, "best_step": self.best_step, "best_metric": self.best_metric,
                    "eval_metric": self.eval_metric, "model": self.model.state_dict(),
                    "optimizer": self.optimizer.state_dict(), "scheduler": self.scheduler.state_dict(), }, save_dir)
                    # "eval_metric": self.eval_metric, "model": self.model.module.state_dict(),
    ...
    def train(self):
        ...
        # if self.local_rank == 0:
        writer = None if self.debug else SummaryCollector(log_dir=self.dir)
            ...
            for batch_idx, train_data in train_generator:
                ...
                with autocast():
                    ...
                    if self.num_classes == 1:
                        ...
                        Y_pre = mindspore.ops.sigmoid(Y_pre)
                    else:
    ...
    def validate(self, epoch, step, timer, writer):
        ...
        with mindspore.no_grad():
            ...
            for val_idx, val_data in val_generator:
                ...
                val_generator.set_description("Eval Epoch %d (%d/%d), Global Step %d, Loss %.4f, ACC %.4f" %
                ...
                if val_idx == v_idx or val_idx == 1:
                    ...
                    # show images
                    images = I[:4]
                    images = mindspore.ops.cat([images, *sample_recons], dim=0)
                    pred = Y_pre[:4]
                    gt = Y[:4]
